...
{
  std::vector<size_t> dims = {(this->header.encoding.front().encodedSpace.matrixSize.z > 1) ? uint(4) : uint(3), traj_input->get_size(1)}; // make the dimensions of traj+dcw be 4 is 3D and 3 is 2D using z encoding to check this
    ...
    Core::InputChannel<Core::variant<Core::Acquisition, Core::Waveform>> &in, Core::OutputChannel &out)
{
  ...
  auto sets = this->header.encoding.front().encodingLimits.set.get().maximum +1;
  kspace_scaling = 1e-3 * fov.x / matrixsize.x;
  auto ksp_lim = this->header.encoding.front().encodingLimits.kspace_encoding_step_1.get().maximum +1;
  std::set<size_t> ksp1;
  int rotations;

  bool set_shots_per_frame = false;
  int shots_per_frame = 0;
  ...
  if (this->header.encoding[0].trajectoryDescription)
  {
    traj_desc = *this->header.encoding[0].trajectoryDescription;
  }
  else
  {
  ...
  try
  {
    ...
    Tsamp_us = userparam_long.at("SamplingTime_ns") * 1e-3;
    if (Tsamp_us == 0)
      Tsamp_us = 2; // need this because UTE sequence doesn't setup this variable (2e-6)
 
    GDEBUG_STREAM("sampling_time in ms: " << float(Tsamp_us));
  }
    ...
    for (auto message : in)
    {
      ...
      if (Core::holds_alternative<Core::Acquisition>(message))
      {
        ...
        if (generateTraj)
        {
          ...
          {
            if (header.encoding[0].encodedSpace.matrixSize.z >= 1 && realTime)
            {
              // GDEBUG_STREAM("acquisitionsVec.size():" <<acquisitionsVec.size());
              // GDEBUG_STREAM(" (ksp_lim * matrixsize.z * (sets+1)):" << (ksp_lim * matrixsize.z * (sets+1)));

              if (acquisitionsVec.size() < (ksp_lim * matrixsize.z * (sets+1)))
              {

                if (!set_shots_per_frame)
                  shots_per_frame++;
                if (!set_shots_per_frame && ((matrixsize.z<=1 && head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_SLICE)) || (matrixsize.z>1 && ( head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_SLICE) || head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_FIRST_IN_REPETITION) || head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_MEASUREMENT) || head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_AVERAGE)))))
                {
                  set_shots_per_frame = true;
                  shots_per_frame = int(shots_per_frame/(sets * header.encoding[0].encodedSpace.matrixSize.z));
                  GDEBUG_STREAM("shots_per_frame:" <<shots_per_frame);

                }
                ksp1.insert(head.idx.kspace_encode_step_1);
                //GDEBUG_STREAM("ksp1.size():" <<ksp1.size());

                acquisitionsVec.push_back(Core::Acquisition(std::move(head), std::move(data), std::move(traj)));
                continue;
                // ksp1.push_back(head.idx.kspace_encode_step_1);
              }
              else
              {
                auto rotations = ksp1.size();
            ...
            else
            {
              ...
              trajParams.set_acceleration_factor(acceleration_factor);
              GadgetronTimer timer("Trajectory Gen:");
              tw_gen = trajParams.calculate_trajectories_and_weight(head);
              traj_not_generated = false;
              trajgen = std::get<0>(tw_gen);
          ...
          acquisitionsVec.push_back(Core::Acquisition(std::move(head), std::move(data), std::move(traj)));
          for (auto acq : acquisitionsVec)
          {
            auto [head, data, trajx] = acq;
        ...
        else
        {
          ...
          if (attachWaveform)
          {
            ...
            // zencoding required to be calculated for each acquisition 
...
void WaveformToTrajectory::prepare_trajectory_from_waveforms(Core::Waveform &grad_waveform, const ISMRMRD::AcquisitionHeader &head)
{
  ...
  trajectory_and_weights(1, 0) = (gradients_interpolated(0)[1]) * GAMMA * 10 * head.sample_time_us * 1e-6 * kspace_scaling;
  trajectory_and_weights(2, 0) = zencoding;
 
  for (int ii = 1; ii < size_gradOVS; ii++)
  {
 
    trajectory_and_weights(0, ii) = ((gradients_interpolated(ii)[0]) * GAMMA * 10 * head.sample_time_us * 1e-6 * kspace_scaling + trajectory_and_weights(0, ii - 1)); // mT/m * Hz/G * 10G * 2e-6
    trajectory_and_weights(1, ii) = ((gradients_interpolated(ii)[1]) * GAMMA * 10 * head.sample_time_us * 1e-6 * kspace_scaling + trajectory_and_weights(1, ii - 1)); // mT/m * Hz/G * 10G * 2e-6
...
void WaveformToTrajectory::applyGIRF(hoNDArray<float> &trajectory_and_weights, ISMRMRD::AcquisitionHeader head, ISMRMRD::IsmrmrdHeader header, float kspace_scaling, hoNDArray<std::complex<float>> girf_kernel)
{
  ...
  if (header.encoding.front().encodedSpace.matrixSize.z > 1)
  {
    ...
    for (int ii = 1; ii < trajectory_and_weights.get_size(1); ii++)
    {
      ...
      trajectory_and_weights(0, ii) = ((gradients(ii)[0]) * GAMMA * 10 * head.sample_time_us * 1e-6 * kspace_scaling + trajectory_and_weights(0, ii - 1)); // mT/m * Hz/G * 10G * 2e-6
      trajectory_and_weights(1, ii) = ((gradients(ii)[1]) * GAMMA * 10 * head.sample_time_us * 1e-6 * kspace_scaling + trajectory_and_weights(1, ii - 1)); // mT/m * Hz/G * 10G * 2e-6
      trajectory_and_weights(2, ii) = zencoding;                                                                                                           // Code to gen 3D traj forDavid
